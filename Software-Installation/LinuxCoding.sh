...
# Check if the MongoDB GPG key file already exists and remove it to avoid the prompt
if [ -f /etc/apt/trusted.gpg.d/mongodb-6.gpg ]; then
   echo "MongoDB GPG key already exists. Removing and replacing with the latest version..."
   sudo rm /etc/apt/trusted.gpg.d/mongodb-6.gpg
fi
# Import the MongoDB GPG key
curl -fsSL https://www.mongodb.org/static/pgp/server-6.0.asc | sudo gpg --dearmor -o /etc/apt/trusted.gpg.d/mongodb-6.gpg
# Add the MongoDB repository
echo "deb [ arch=amd64,arm64 ] https://repo.mongodb.org/apt/ubuntu focal/mongodb-org/6.0 multiverse" | sudo tee /etc/apt/sources.list.d/mongodb-org-6.0.list
# Install a specific version of libssl required by MongoDB if not available in the default repo
cd ~/Downloads || return
